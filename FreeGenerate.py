...
import function.generate.FreeDataInput as FreeInput
...
Logpath = generate_args.Logpath + \
    '/{}/'.format(PathName)   # 読み込む学習済みモデルのフォルダのパス
...
FreeInputData = FreeInput.InputFreeData()
InputList, NameList = FreeInputData.SetData(ModelName, PathName)
###############################################################################
#　モデルを構築
###############################################################################
...
model.load_state_dict(torch.load(
    path_model, map_location=torch.device(device)))
model.eval()
...
DataSelect = getattr(Rulefn,
                     generate_args.DataSelect)(corpus, device,
                                               ...
                                               # 使用する音名の範囲(最低音)
                                               startmidinum=UseMidiNum[0],
                                               # 使用する音名の範囲(最高音)
                                               endmidinum=UseMidiNum[1],
...
for musicidx, (Data, Name) in enumerate(zip(InputList, NameList)):
    ...
    Generate_root = TreeSearch.generate(Pitch=Pitch, Duration=Duration, Offset=Tick)      # 木探索による生成
